...
if __name__ == "__main__":
    ...
    # tdir = sys.argv[1]
    plotsavepath = sys.argv[2]
    patchname = sys.argv[1] #sys.argv[1]=datapath
    dirname = os.path.dirname(patchname) 
    dirname = "."
    tag = sys.argv[3] # os.path.basename(patchname).replace(".h5", "")
    title = tag.replace("_", ", ")

    tfig, ax = plot_trace(patchname)
    tfig.suptitle(title)
    tfig.tight_layout()
    tfig.savefig(f"{plotsavepath}/{tag}_trace.png", dpi=200)
    pl.close(tfig)

    cfig, caxes = plot_corner(patchname)
    cfig.text(0.4, 0.8, title, transform=cfig.transFigure)
    cfig.savefig(f"{plotsavepath}/{tag}_corner.png", dpi=200)
    pl.close(cfig)

    rfig, raxes, rcb, val = plot_residual(patchname)
    rfig.savefig(f"{plotsavepath}/{tag}_residual.png", dpi=200)
    pl.close(rfig)
